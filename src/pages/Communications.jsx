    ...
    <div style={{ display: 'grid', gap: 22 }}>
      ...
      <section style={{ padding: 24, border: '1px solid var(--border-default)', borderRadius: 18, background: 'var(--surface-card)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: 18, alignItems: 'flex-start', flexWrap: 'wrap' }}>
          <div>
            ...
            <h1 style={{ fontFamily: 'var(--font-display)', fontSize: 28, color: 'var(--color-navy-900)', margin: '8px 0 0' }}>Announcements, updates and certificate delivery</h1>
      ...
      {tab === 'compose' && (
        <div className="lex-comm-grid">
          <section style={{ padding: 22, border: '1px solid var(--border-default)', borderRadius: 14, background: 'var(--surface-card)' }}>
            ...
            <label htmlFor="comm-template" style={{ fontSize: 12, fontWeight: 700 }}>Template</label>
            <select id="comm-template" value={template} onChange={e => chooseTemplate(e.target.value)} style={{ ...input, marginTop: 6 }}>{Object.entries(templates).map(([key, value]) => <option key={key} value={key}>{value.label}</option>)}</select>
            <label htmlFor="comm-subject" style={{ display: 'block', fontSize: 12, fontWeight: 700, marginTop: 14 }}>Subject</label>
            <input id="comm-subject" value={subject} onChange={e => setSubject(e.target.value)} style={{ ...input, marginTop: 6 }}/>
            <label htmlFor="comm-message" style={{ display: 'block', fontSize: 12, fontWeight: 700, marginTop: 14 }}>Message</label>
            <textarea id="comm-message" value={message} onChange={e => setMessage(e.target.value)} rows={9} style={{ ...input, marginTop: 6, resize: 'vertical', lineHeight: 1.55 }}/>
          ...
          <aside style={{ padding: 20, border: '1px solid var(--border-default)', borderRadius: 14, background: 'var(--surface-card)' }}>
            ...
            <div className="lex-comm-filters" style={{ marginTop: 14 }}>
              <div><label htmlFor="comm-audience-activity" style={{ fontSize: 12, fontWeight: 700 }}>Activity</label><select id="comm-audience-activity" style={{ ...input, marginTop: 5 }} value={audience.activityId} onChange={e => setAudience(a => ({ ...a, activityId: e.target.value }))}><option value="all">All activities</option>{activities.map(a => <option key={a.id} value={String(a.id)}>{a.title}</option>)}</select></div>
              <div><label htmlFor="comm-audience-participant" style={{ fontSize: 12, fontWeight: 700 }}>Specific participant</label><select id="comm-audience-participant" style={{ ...input, marginTop: 5 }} value={audience.participantId} onChange={e => setAudience(a => ({ ...a, participantId: e.target.value }))}><option value="all">Everyone matching filters</option>{participants.map(p => <option key={p.id} value={String(p.id)}>{p.name}</option>)}</select></div>
              <div><label htmlFor="comm-audience-category" style={{ fontSize: 12, fontWeight: 700 }}>Category</label><select id="comm-audience-category" style={{ ...input, marginTop: 5 }} value={audience.category} onChange={e => setAudience(a => ({ ...a, category: e.target.value }))}><option value="all">All categories</option>{categories.map(value => <option key={value}>{value}</option>)}</select></div>
              <div><label htmlFor="comm-audience-organisation" style={{ fontSize: 12, fontWeight: 700 }}>Organisation</label><select id="comm-audience-organisation" style={{ ...input, marginTop: 5 }} value={audience.organization} onChange={e => setAudience(a => ({ ...a, organization: e.target.value }))}><option value="all">All organisations</option>{organizations.map(value => <option key={value}>{value}</option>)}</select></div>
            </div>
      ...
      {tab === 'history' && (
        <div style={{ display: 'grid', gap: 14 }}>
          ...
          <div className="lex-comm-history-tools">
            <label className="lex-comm-search"><Search size={15}/><span className="lex-comm-sr-only">Search communications</span><input value={historySearch} onChange={event => setHistorySearch(event.target.value)} placeholder="Search subject or type"/></label>
      ...
      {tab === 'settings' && (
        <section style={{ maxWidth: 700, padding: 22, border: '1px solid var(--border-default)', borderRadius: 14, background: 'var(--surface-card)' }}>
          ...
          <label htmlFor="comm-reply-to" style={{ display: 'block', fontSize: 12, fontWeight: 700, marginTop: 20 }}>Reply-to email</label>
          <input id="comm-reply-to" type="email" value={replyToEmail} disabled={previewReadOnly} onChange={e => setReplyToEmail(e.target.value)} placeholder="programmes@yourorganisation.org" style={{ ...input, marginTop: 6 }}/>
          <div style={{ marginTop: 7, fontSize: 12, color: 'var(--text-tertiary)', lineHeight: 1.5 }}>Participant replies will go to this address. Sending still uses the authenticated LexAMS delivery domain.</div>
      ...
      {showSendReview && <div className="lex-comm-review-backdrop" role="presentation" onMouseDown={event => event.target === event.currentTarget && setShowSendReview(false)}>
        <section className="lex-comm-review" role="dialog" aria-modal="true" aria-labelledby="communication-review-title">
          ...
          <dl style={{ display: 'grid', gap: 10, marginTop: 18, padding: 16, borderRadius: 10, background: 'var(--surface-muted)', fontSize: 13 }}>
            <div><dt style={{ color: 'var(--text-tertiary)', fontSize: 12, textTransform: 'uppercase' }}>Recipients</dt><dd style={{ marginTop: 3, fontWeight: 800 }}>{matchedParticipants.length} participant{matchedParticipants.length === 1 ? '' : 's'}</dd></div>
            <div><dt style={{ color: 'var(--text-tertiary)', fontSize: 12, textTransform: 'uppercase' }}>Subject</dt><dd style={{ marginTop: 3, fontWeight: 700 }}>{subject}</dd></div>
            <div><dt style={{ color: 'var(--text-tertiary)', fontSize: 12, textTransform: 'uppercase' }}>Audience</dt><dd style={{ marginTop: 3 }}>{audience.participantId !== 'all' ? participants.find(participant => String(participant.id) === audience.participantId)?.name : 'Everyone matching the selected filters'}</dd></div>
          </dl>
