...
function SessionDialog({ initial, activity, facilitators, saving, onClose, onSave }) {
  const [form, setForm] = useState(() => ({
    ...
    facilitator_id: String(initial?.facilitator_id || ''),
  }));
  const selectedFacilitator = facilitators.find(facilitator => String(facilitator.id) === form.facilitator_id);
  ...
  return <div className="planning-modal-backdrop" onMouseDown={event => event.target === event.currentTarget && !saving && onClose()}>
    <section className="planning-modal planning-modal-large" role="dialog" aria-modal="true" aria-labelledby="planning-session-title">
      ...
      <div className="planning-form-grid">
        <label className="wide"><span>Session name</span><input autoFocus value={form.title} onChange={event => setForm({ ...form, title: event.target.value })} placeholder="Opening and participant orientation"/></label>
        ...
        <label><span>Start time</span><input type="time" value={form.starts_at} onChange={event => setForm({ ...form, starts_at: event.target.value })}/></label>
        <label><span>End time</span><input type="time" value={form.ends_at} onChange={event => setForm({ ...form, ends_at: event.target.value })}/></label>
        <label className="wide"><span>Venue</span><input value={form.venue} onChange={event => setForm({ ...form, venue: event.target.value })} placeholder="Main training room"/></label>
        <label className="wide"><span>Facilitator</span><select value={form.facilitator_id} onChange={event => setForm({ ...form, facilitator_id: event.target.value })}><option value="">Not assigned</option>{facilitators.map(facilitator => <option key={facilitator.id} value={String(facilitator.id)}>{facilitator.name} · {facilitator.role}</option>)}</select></label>
        <label className="wide"><span>Facilitator email</span><input readOnly value={selectedFacilitator?.email || ''} placeholder={facilitators.length ? 'Select a facilitator above' : 'Add a facilitator from the Facilitators tab'}/></label>
      </div>
...
function SessionCard({ session, canManage, canUpdateStatus, saving, onEdit, onStatus }) {
  ...
  return <article className="planning-session-card">
    ...
    <div className="planning-session-body"><div className="planning-session-top"><span className={`planning-status status-${session.planning_status}`}>{session.planning_status}</span>{canManage && <button className="planning-icon-button" onClick={onEdit} aria-label={`Edit ${session.title}`}><Edit3 size={14}/></button>}</div><h5>{session.title}</h5>{session.description && <p>{session.description}</p>}
      <div className="planning-session-meta"><span><Clock3 size={13}/>{timeValue(session.starts_at) || 'Time not set'}{session.ends_at ? `–${timeValue(session.ends_at)}` : ''}</span><span><MapPin size={13}/>{session.venue || 'Venue not set'}</span><span><UserRound size={13}/>{lead?.name || 'No facilitator'}</span>{lead?.email && <span>{lead.email}</span>}</div>
...
export default function PlanningSessions({ data, saving, onMutate, onUpgrade }) {
  ...
  return <div className="planning-section-stack">
    <div className="planning-toolbar"><div><h4>Sessions</h4><p>Manage each session’s name, facilitator, venue, date and time.</p></div><div className="planning-toolbar-actions">{multiWeek && <div className="planning-view-toggle" role="group" aria-label="Session layout"><button className={viewMode === 'week' ? 'active' : ''} onClick={() => setViewMode('week')}><CalendarRange size={14}/>Week</button><button className={viewMode === 'list' ? 'active' : ''} onClick={() => setViewMode('list')}><List size={14}/>List</button></div>}<select value={filter} onChange={event => { setFilter(event.target.value); setActiveWeek(0); }} aria-label="Filter session status"><option value="all">All planning states</option>{statusOptions.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select>{(canManage || data.permissions.readOnlyPreview) && (canImportCsv ? <PlanningSessionCsvImport activity={data.activity} facilitators={data.facilitators} saving={saving} onMutate={onMutate} preview={data.permissions.readOnlyPreview}/> : <button className="planning-secondary-button planning-pro-button" onClick={onUpgrade}><LockKeyhole size={14}/>Import CSV · Pro</button>)} {canManage && <button className="planning-primary-button" onClick={() => setDialog({})}><Plus size={15}/>Add session</button>}</div></div>
    ...
    {dialog && <SessionDialog initial={dialog.id ? dialog : null} activity={data.activity} facilitators={data.facilitators} saving={saving} onClose={() => setDialog(null)} onSave={save}/>}
  </div>;
}
