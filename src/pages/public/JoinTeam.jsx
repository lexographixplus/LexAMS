...
export default function JoinTeam(){
 ...
 const accept = useCallback(async()=>{setAccepting(true);setError('');try{const r=await fetch(`/api/invite/${token}`,{method:'POST',credentials:'include'});const b=await r.json();if(!r.ok)throw new Error(b.error);await refreshProfile();setAccepted(true)}catch(e){setError(e.message)}finally{setAccepting(false)}},[token,refreshProfile]);
 useEffect(()=>{if(invite&&user&&!authLoading&&!accepted&&!accepting&&invite.status==='pending')void accept()},[invite,user,authLoading,accepted,accepting,accept]);
