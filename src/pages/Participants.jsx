...
export default function Participants() {
  ...
  return <div>
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 20 }}>
      <div><h2 style={{ fontFamily: 'var(--font-display)', fontSize: 24, fontWeight: 700 }}>Participants</h2><p style={{ fontSize: 13, color: 'var(--text-secondary)', marginTop: 6 }}>Central database of everyone engaged across activities.</p></div>
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 220px', gap: 14, marginTop: 22 }}>
      <input placeholder="Search by name, email or organization..." value={q} onChange={e => setQ(e.target.value)} style={inputStyle}/>
      <select value={catF} onChange={e => setCatF(e.target.value)} style={inputStyle}>{categories.map(c => <option key={c} value={c}>{c === 'all' ? 'All categories' : c}</option>)}</select>
    </div>
    ...
    <div style={{ background: 'var(--surface-card)', border: '1px solid var(--border-default)', borderRadius: 'var(--radius-lg)', boxShadow: 'var(--shadow-card)', overflow: 'hidden', marginTop: 18 }}>
      <div className="table-scroll"><div style={{ minWidth: 700 }}>
        <div style={{ display: 'grid', gridTemplateColumns: '2fr 1.2fr 1.6fr 1.1fr 0.8fr 0.8fr', gap: 14, padding: '12px 22px', fontSize: 11, letterSpacing: '0.07em', textTransform: 'uppercase', color: 'var(--text-tertiary)', fontWeight: 600, background: 'var(--surface-muted)' }}><div>Participant</div><div>Phone</div><div>Organization</div><div>Category</div><div>Activities</div><div>Certs</div></div>
        {filtered.map(p => {
          ...
          return <div key={p.id} onClick={() => setSelectedPid(p.id)} style={{ display: 'grid', gridTemplateColumns: '2fr 1.2fr 1.6fr 1.1fr 0.8fr 0.8fr', gap: 14, alignItems: 'center', padding: '12px 22px', borderTop: '1px solid var(--border-default)', cursor: 'pointer' }} onMouseEnter={e => e.currentTarget.style.background = 'var(--surface-muted)'} onMouseLeave={e => e.currentTarget.style.background = ''}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}><div style={{ width: 32, height: 32, borderRadius: 999, background: 'var(--surface-muted)', color: 'var(--color-navy-700)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 700, flexShrink: 0 }}>{getInitials(p.name)}</div><div><div style={{ fontSize: 13, fontWeight: 600 }}>{p.name}</div><div style={{ fontSize: 12, color: 'var(--text-tertiary)' }}>{p.email}</div></div></div>
