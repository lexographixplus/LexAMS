...
export default function ReportSectionEditor({ section, report, permissions, commercial, saving, onMutate, onUpgrade, onMove, onSelectAfterDelete }) {
  const [draft, setDraft] = useState(() => ({ title: section.title, content_text: section.content_text || '', instructions: section.instructions || '' }));
  const entitlements = commercial?.entitlements || {};
  const roleEditable = permissions.canEditReports && !permissions.readOnlyPreview;
  const workflowFrozen = !entitlements.reportApprovals && report.status !== 'draft';
  const editable = roleEditable && !workflowFrozen;
  const canEditStructure = editable && Boolean(entitlements.reportStructureEditing);
  const canGenerate = editable && Boolean(entitlements.narrativeGeneration);
  const canApprove = editable && Boolean(entitlements.reportApprovals);
  ...
  return <section className="activity-report-editor-card">
    <header><div><span className={`activity-report-state state-${section.source_changed ? 'stale' : section.content_state}`}>{stateLabel(section)}</span><h4>{section.title}</h4><p>{section.section_type.replaceAll('_', ' ')} section{section.source_type ? ` · ${reportSourceLabel(section.source_type)}` : ''}{section.is_required ? ' · Required' : ' · Optional'}</p></div><div className="activity-report-editor-order"><button disabled={!canEditStructure} onClick={() => onMove(-1)} aria-label={`Move ${section.title} up`}>↑</button><button disabled={!canEditStructure} onClick={() => onMove(1)} aria-label={`Move ${section.title} down`}>↓</button></div></header>

    {workflowFrozen && <div className="activity-report-pro-note"><LockKeyhole size={16}/><span>This reviewed report remains readable. Restore Pro to unlock editing and approval controls.</span></div>}
    {section.source_changed && <div className="activity-report-stale-banner"><AlertTriangle size={19}/><div><strong>Source data changed since this narrative was prepared</strong><p>Your current writing has been preserved. Review the change, then refresh the draft or keep the existing text.</p><SourceComparison section={section}/><div>{editable && <>{canGenerate ? <button className="primary" onClick={generate} disabled={saving}><RefreshCw size={14}/>Refresh narrative</button> : <button className="secondary pro-locked" onClick={() => onUpgrade('narrative-generation')}><LockKeyhole size={14}/>Refresh · Pro</button>}<button className="secondary" onClick={() => onMutate('acknowledge_source', { sectionId: section.id }, 'Existing narrative retained and source change acknowledged.')} disabled={saving}>Keep existing</button></>}</div></div></div>}

    <div className="activity-report-editor-fields"><label><span>Section title</span><input value={draft.title} disabled={!canEditStructure || locked} onChange={event => setDraft({ ...draft, title: event.target.value })}/></label><label><span>Author guidance</span><textarea value={draft.instructions} disabled={!canEditStructure || locked} onChange={event => setDraft({ ...draft, instructions: event.target.value })}/></label>{section.section_type !== 'linked' && <label className="narrative"><span>Report narrative</span><textarea value={draft.content_text} disabled={!canWrite} placeholder="Write this section or generate a grounded first draft from the linked records." onChange={event => setDraft({ ...draft, content_text: event.target.value })}/><small>{draft.content_text.length.toLocaleString()} characters · Changes are saved explicitly so approved writing stays protected.</small></label>}</div>
    ...
    <footer><div>{canEditStructure && <button className="danger-text" onClick={remove} disabled={saving || report.sections.length <= 1}><Trash2 size={14}/>Remove section</button>}</div><div>{roleEditable && section.content_state === 'approved' && (canApprove ? <button className="secondary" onClick={() => onMutate('set_section_approval', { sectionId: section.id, approved: false }, 'Section unlocked for editing.')} disabled={saving}><LockKeyhole size={14}/>Unlock</button> : <button className="secondary pro-locked" onClick={() => onUpgrade('report-approvals')}><LockKeyhole size={14}/>Unlock · Pro</button>)}{editable && ['generated', 'hybrid'].includes(section.section_type) && !section.source_changed && (canGenerate ? <button className="secondary" onClick={generate} disabled={saving || !section.has_source_data}><Sparkles size={14}/>{section.content_text ? 'Regenerate' : 'Generate draft'}</button> : <button className="secondary pro-locked" onClick={() => onUpgrade('narrative-generation')}><LockKeyhole size={14}/>Generate · Pro</button>)}{canWrite && <button className="primary" onClick={save} disabled={saving || !draft.title.trim()}><Save size={14}/>Save section</button>}{editable && section.content_state !== 'approved' && (section.section_type === 'linked' ? section.has_source_data : Boolean(draft.content_text.trim())) && (canApprove ? <button className="approve" onClick={() => onMutate('set_section_approval', { sectionId: section.id, approved: true }, 'Section approved and protected.')} disabled={saving}><Check size={14}/>Approve</button> : <button className="secondary pro-locked" onClick={() => onUpgrade('report-approvals')}><LockKeyhole size={14}/>Approve · Pro</button>)}</div></footer>
  </section>;
}
