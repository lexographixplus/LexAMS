...
import { AlertCircle, BookOpenCheck, CheckCircle2, FilePlus2, FileText, LayoutTemplate, LockKeyhole, Pencil, Plus, Printer, RefreshCw, Sparkles, Trash2, X } from 'lucide-react';
import { canCreateActivityReport } from '../../../shared/commercial.js';
...
function CreateReportDialog({ data, initialTemplateId, saving, onClose, onCreate }) {
  const templates = data.commercial?.entitlements?.customReportTemplates ? data.templates : data.templates.filter(template => template.is_builtin);
  const defaultTemplate = templates.find(template => String(template.id) === String(initialTemplateId)) || templates[0];
  ...
  const selected = templates.find(template => String(template.id) === String(form.template_id));
...
function ReportDetailsDialog({ report, saving, workflowEnabled, onClose, onSave, onDelete }) {
  ...
  const frozen = !workflowEnabled && report.status !== 'draft';
  return <DialogShell title="Report details" kicker="Activity report" description="Update the report identity, period and review status." onClose={onClose} footer={<><button className="danger-text" onClick={onDelete} disabled={saving}><Trash2 size={14}/>Delete report</button><div><button className="secondary" onClick={onClose}>Cancel</button><button className="primary" onClick={() => onSave(form)} disabled={saving || frozen || !form.title.trim()}>{saving ? 'Saving…' : 'Save details'}</button></div></>}>
    {frozen && <div className="activity-report-pro-note"><LockKeyhole size={16}/><span>This report remains readable after downgrade. Restore Pro to edit a report that is in review or approved.</span></div>}
    <div className="activity-report-form-grid"><label className="wide"><span>Report title</span><input value={form.title} disabled={frozen} onChange={event => setForm({ ...form, title: event.target.value })}/></label><label><span>Status</span><select value={form.status} disabled={!workflowEnabled} onChange={event => setForm({ ...form, status: event.target.value })}><option value="draft">Draft</option>{!workflowEnabled && report.status !== 'draft' && <option value={report.status}>{report.status.replaceAll('_', ' ')}</option>}{workflowEnabled && <option value="in_review">In review</option>}{workflowEnabled && <option value="approved">Approved</option>}</select></label><label><span>Period starts</span><input type="date" value={form.reporting_period_start} disabled={frozen} onChange={event => setForm({ ...form, reporting_period_start: event.target.value })}/></label><label><span>Period ends</span><input type="date" min={form.reporting_period_start} value={form.reporting_period_end} disabled={frozen} onChange={event => setForm({ ...form, reporting_period_end: event.target.value })}/></label></div>
  </DialogShell>;
}
...
export default function ActivityReportWorkspace({ activity }) {
  ...
  const entitlements = data?.commercial?.entitlements || {};
  const isFree = data?.commercial?.plan === 'free';
  const canCreateMore = canCreateActivityReport(data?.commercial?.usage?.activityReports || 0, entitlements);
  const visibleTemplates = entitlements.customReportTemplates ? (data?.templates || []) : (data?.templates || []).filter(template => template.is_builtin);
  const upgrade = feature => navigate(`/app/checkout?feature=${encodeURIComponent(feature)}`);
  ...
  return <div className="activity-report-workspace">
    ...
    {isFree && <div className="activity-report-message commercial"><LockKeyhole size={17}/><span><strong>Your basic activity report is included.</strong> Use built-in templates, linked evidence, manual writing and Print/PDF. Pro adds multiple reports, custom templates, grounded draft generation and approval workflows.</span><button onClick={() => upgrade('living-reporting')}>View Pro</button></div>}
    ...
    {!report ? <section className="activity-report-empty"><FileText size={31}/><h4>Start the living report</h4><p>Choose a reusable template now. Linked sections will fill from activity records as delivery progresses.</p><div className="activity-report-empty-templates">{visibleTemplates.slice(0, 3).map(template => <article key={template.id}><LayoutTemplate size={18}/><strong>{template.name}</strong><span>{template.sections.length} sections</span><p>{template.description}</p>{data.permissions.canCreateReports && canCreateMore && <button onClick={() => setDialog({ type: 'create', templateId: template.id })}>Use template</button>}</article>)}</div></section> : <>
      <section className="activity-report-toolbar"><div className="activity-report-switcher"><label><span>Activity report</span><select value={report.id} onChange={event => { setSelectedReportId(event.target.value); setSelectedSectionId(null); }}>{data.reports.map(item => <option key={item.id} value={item.id}>{item.title}</option>)}</select></label><span className={`activity-report-status status-${report.status}`}>{report.status.replaceAll('_', ' ')}</span></div><div className="activity-report-actions"><div className="activity-report-view-toggle" role="group" aria-label="Report view"><button className={view === 'write' ? 'active' : ''} onClick={() => setView('write')}>Write</button><button className={view === 'preview' ? 'active' : ''} onClick={() => setView('preview')}>Preview</button></div>{(data.permissions.canManageTemplates || data.permissions.readOnlyPreview) && <button onClick={() => setDialog({ type: 'templates' })}><LayoutTemplate size={14}/>Templates{!entitlements.customReportTemplates && <span className="activity-report-pro-label">Pro editing</span>}</button>}{data.permissions.canCreateReports && (canCreateMore ? <button onClick={() => setDialog({ type: 'create' })}><FilePlus2 size={14}/>New report</button> : <button className="pro-locked" onClick={() => upgrade('multiple-activity-reports')}><LockKeyhole size={14}/>More reports · Pro</button>)}{data.permissions.canEditReports && <button onClick={() => setDialog({ type: 'details' })}><Pencil size={14}/>Details</button>}<button className="primary" onClick={() => printActivityReport(report.title)}><Printer size={14}/>Print / PDF</button></div></section>
      ...
      {view === 'write' && <div className="activity-report-write-layout"><aside className="activity-report-outline"><header><div><strong>Report outline</strong><small>{report.completion.requiredIncomplete} required section{report.completion.requiredIncomplete === 1 ? '' : 's'} remaining</small></div>{data.permissions.canEditReports && (entitlements.reportStructureEditing ? <button onClick={() => setDialog({ type: 'add-section' })} aria-label="Add report section"><Plus size={15}/></button> : <button className="pro-locked" onClick={() => upgrade('custom-report-structures')} aria-label="Upgrade to add report sections"><LockKeyhole size={14}/></button>)}</header><nav aria-label="Report sections">{report.sections.map((item, index) => <button key={item.id} className={section?.id === item.id ? 'active' : ''} onClick={() => setSelectedSectionId(item.id)}><span>{String(index + 1).padStart(2, '0')}</span><div><strong>{item.title}</strong><small>{item.source_changed ? 'Source changed' : item.content_state === 'approved' ? 'Approved' : item.section_type === 'linked' && item.has_source_data ? 'Live data' : item.content_state.replaceAll('_', ' ')}</small></div><i className={item.source_changed ? 'stale' : item.content_state === 'approved' || (item.section_type === 'linked' && item.has_source_data) ? 'complete' : ''}/></button>)}</nav></aside>{section && <ReportSectionEditor key={`${section.id}:${section.updated_at}:${section.content_state}:${section.source_hash || ''}`} section={section} report={report} permissions={data.permissions} commercial={data.commercial} saving={saving} onMutate={mutate} onUpgrade={upgrade} onMove={moveSection} onSelectAfterDelete={() => setSelectedSectionId(report.sections.find(item => item.id !== section.id)?.id || null)}/>}</div>}
    ...
    {dialog?.type === 'templates' && <ReportTemplateManager data={data} saving={saving} onClose={() => setDialog(null)} onMutate={mutate} onUpgrade={upgrade}/>}
    ...
    {dialog?.type === 'details' && <ReportDetailsDialog
      report={report}
      saving={saving}
      workflowEnabled={Boolean(entitlements.reportApprovals)}
      onClose={() => setDialog(null)}
      onSave={async form => { const result = await mutate('save_report', { report: form }, 'Report details updated.'); if (result) setDialog(null); }}
      onDelete={async () => { if (!window.confirm(`Delete “${report.title}” and all of its sections?`)) return; const result = await mutate('delete_report', { reportId: report.id }, 'Report deleted.'); if (result) { setSelectedReportId(null); setSelectedSectionId(null); setDialog(null); } }}
    />}
